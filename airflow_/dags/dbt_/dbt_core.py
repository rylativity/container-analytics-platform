...
from .dbt_adapters import Adapter
...
def collect_data_from_dbt_core(dbt_project_path: str, dbt_profiles_yaml_path: str = None):
    """
    Collect metadata for a single run of dbt core command.
    Args:
        dbt_project_path: Path (on local fs) where dbt project is located.

        dbt_profiles_yaml_path (Optional): Path (on local fs) where dbt profiles.yml file is located.
    """
    ...
    try:
        phase = "load"
        assets = _load_dbt_core_assets(dbt_project_path=dbt_project_path, dbt_profiles_yaml_path=dbt_profiles_yaml_path)
...
def _load_dbt_core_assets(dbt_project_path: str, dbt_profiles_yaml_path: str) -> "DbtCoreAssets":
    ...
    profile = _read_profile(runs_info, project, dbt_profiles_yaml_path)
...
def _read_profile(runs_info: Dict, project: Dict, dbt_profiles_yaml_path: str) -> Dict:
    try:
        profile_name = project["profile"]
        if dbt_profiles_yaml_path is None:
            profile_dir = runs_info["args"]["profiles_dir"]
            profiles_file_path = os.path.join(profile_dir, "profiles.yml")
        else:
            profiles_file_path = dbt_profiles_yaml_path
